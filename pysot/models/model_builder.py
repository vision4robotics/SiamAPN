...
class ModelBuilder(nn.Module):
    ...
    def getcenter(self,mapp):
        ...
        def con(x):
            return x*cfg.TRACK.INSTANCE_SIZE//2
        
        size=mapp.size()[3]
        number=cfg.TRACK.INSTANCE_SIZE//2-size//2*cfg.TRACK.STRIDE
        x=np.tile((cfg.TRACK.STRIDE*(np.linspace(0,size-1,size))+number)-cfg.TRACK.INSTANCE_SIZE//2,size).reshape(-1)
        y=np.tile((cfg.TRACK.STRIDE*(np.linspace(0,size-1,size))+number).reshape(-1,1)-cfg.TRACK.INSTANCE_SIZE//2,size).reshape(-1)
        ...
        anchor[:,:,0]=x+cfg.TRACK.INSTANCE_SIZE//2
        anchor[:,:,1]=y+cfg.TRACK.INSTANCE_SIZE//2
        anchor[:,:,2]=w
        anchor[:,:,3]=h
